...
function getFizzBuzz(num) {
  ...
  function isBracketsBalanced(str) {
    ...
    const pairToCloseBracket = Object.assign(...bracketsConfig.map(function(el) {
        return { [el[1]]: el[0] };
    }));
...
function toNaryString(num, n) {
  ...
  function getCommonDirectoryPath(pathes) {
    const at = (i) => (a) => a[i];
    const splittedArr = pathes.map((el) => el.replace(/\//g, '/ ').split(' '));
    const rotatedArr = splittedArr[0].map((el, i) => splittedArr.map(at(i)));
    const filterEqPath = (arr) => arr.every((e) => e === arr[0]);
    const filteredArr = rotatedArr.filter(filterEqPath);
    return filteredArr.map(at(0)).join('');
  }
