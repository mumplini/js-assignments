...
function getRectangleString(width, height) {
  ...
  + INNER_LINE.replace(' ', ' '.repeat(width - 2)).repeat(height - 2)
...
function encodeToRot13(str) {
  const INPUT_DICT = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const OUTPUT_DICT = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  return str.split('').map((el) => {
    if (/[a-z]/ig.test(el)) return OUTPUT_DICT.charAt(INPUT_DICT.indexOf(el));
    return el;
  }).join('');
}
...
function getCardId(value) {
    ...
    return INITIAL_DECK.indexOf(value);
}
